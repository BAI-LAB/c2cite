...
project_path = "/yy21/MoE-PEFT/tblogs"
name = "results"
...
class TrainConfig(DispatcherConfig):
    ...
    def from_config(config: Dict[str, any]):
        ...
        return TrainConfig(
            ...
            atten_coin=config.get("atten_mat_coin", None),
            router_coin=config.get("router_coin", None),
            cite_coin=config.get("cite_coin", None),
    ...
    def _dataload_fn(self, tokenizer: Tokenizer, **tokenizer_kwargs):
        ...
        for idx, data_point in enumerate(data):
            ...
            data_point.tokens.append(tokenizer.pad_id_)
            if data_point.citations is not None:
                if data_point.citation_embeds is None:
                    citetokens = []
                    for c in data_point.citations:
                        citetokens.append(tokenizer.encode(c, **tokenizer_kwargs))
                    data_point.citation_tokens = citetokens
                data_point.token_len = len(data_point.tokens)
                data_point.prompt_len = len(tokenizer.encode(data_point.prompt, **tokenizer_kwargs))
            if idx % 10000 == 0:
...
def _compute_loss(config_dict: Dict[str, TrainConfig], outputs: List[LLMModelOutput], steps, tb_writer):
    total_loss = None
    for output in outputs:
        ...
        tb_writer.add_scalar('main_loss', loss, steps)
        if output.aux_loss:
            aux_loss = output.aux_loss / config_dict[adapter_name].accumulation_step_
            logging.info(f"    adapter: {adapter_name}  aux: {aux_loss}")
            tb_writer.add_scalar('aux_loss', aux_loss, steps)
            loss += aux_loss 
        if total_loss is None:
            total_loss = loss
        else:
            total_loss += loss
        tb_writer.add_scalar('total_loss', total_loss, steps)
    return total_loss
...
def train(
    ...
    log_dir = os.path.join(tb_writer_summary_path, current_time + f'{configs[0].adapter_name}')
    ...
    while not dispatcher.check_task_done():
        ...
        total_loss = _compute_loss(config_dict, outputs, steps, tb_writer)
